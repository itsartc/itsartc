 ...
 *
 * Until real accounts exist (a later phase), each browser is assigned a random
 * but stable "guest" identity, persisted in localStorage. Opening a second tab
 * or a friend's browser yields a distinct person — enough to feel the live
 * world today, and shaped exactly like the eventual account profile so the
 * networking layer doesn't change when accounts land.
 */
...
const SKIN = ["#f2c6a0", "#e8b48c", "#c68642", "#8d5524", "#ffdbac", "#a1665e"];
const HAIR = ["#2a2a2a", "#4a2f1b", "#6b4423", "#111", "#c9a227", "#7a3b2e", "#3b3b6b"];
const SHIRT = ["#c33c3c", "#2f6fb0", "#3fa66a", "#d9a441", "#8a4fc0", "#e06c9f", "#2f3b4a"];
const PANTS = ["#2f3b4a", "#3a3a3a", "#4a3b2a", "#264653", "#5a5a5a"];
...
function generate(): PlayerIdentity {
  ...
  return {
    ...
    palette: { skin: pick(SKIN), hair: pick(HAIR), top: pick(SHIRT), bottom: pick(PANTS) },
  };
}

/**
 * Returns this browser's stable guest identity, creating and persisting one on
 * first visit. Safe to call before hydration — falls back to a fresh identity
...
export function getLocalIdentity(): PlayerIdentity {
  ...
  try {
    ...
    if (raw) {
      ...
      if (parsed && parsed.id && parsed.name && parsed.palette) return parsed;
    }
  } catch {
